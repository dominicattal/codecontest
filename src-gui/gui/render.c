...
{
    size_t size;
    gui_context.data.capacity += FLOATS_PER_COMP * (num_comps+100);
    size = gui_context.data.capacity * sizeof(GLfloat);
    if (gui_context.data.buffer == NULL) {
        gui_context.data.buffer = st_malloc(size);
        assert(gui_context.data.buffer != NULL);
    } else {
        GLfloat* buf = st_realloc(gui_context.data.buffer, size); 
        assert(buf != NULL);
        gui_context.data.buffer = buf;
    }
}

static void push_quad_data(Quad* quad)
{
    if (gui_context.data.length + FLOATS_PER_COMP >= gui_context.data.capacity)
        resize_data_buffer(100);
    gui_context.data.buffer[gui_context.data.length++] = quad->x;
    gui_context.data.buffer[gui_context.data.length++] = quad->y;
    gui_context.data.buffer[gui_context.data.length++] = quad->w;
    gui_context.data.buffer[gui_context.data.length++] = quad->h;
    gui_context.data.buffer[gui_context.data.length++] = quad->r;
    gui_context.data.buffer[gui_context.data.length++] = quad->g;
    gui_context.data.buffer[gui_context.data.length++] = quad->b;
    gui_context.data.buffer[gui_context.data.length++] = quad->a;
    gui_context.data.buffer[gui_context.data.length++] = quad->u1;
    gui_context.data.buffer[gui_context.data.length++] = quad->v1;
    gui_context.data.buffer[gui_context.data.length++] = quad->u2;
    gui_context.data.buffer[gui_context.data.length++] = quad->v2;
    gui_context.data.buffer[gui_context.data.length++] = quad->location;
    gui_context.data.instance_count++;
}
...
static void push_text_data(GUIComp* comp, i32 cx, i32 cy, i32 cw, i32 ch)
{
    ...
    vbo_idx = gui_context.data.length;
    while (right < length) {
    ...
    while (vbo_idx < gui_context.data.length) {
        gui_context.data.buffer[vbo_idx + 1] -= dy;
...
{
    gui_context.data.instance_count = 0;
    gui_context.data.length = 0;
...
void gui_render(void)
{
    ...
    glDrawArraysInstanced(GL_TRIANGLE_STRIP, 0, 4, gui_context.data.instance_count);
}

 ...
 * 1. An initila scan of the quotient constraints to see if they restrict the
 ...
 *		scan through the quotient constraints to see if they further restrict the
...
{
	int i, con, row, col, baseConsRow, baseConsCol, con_cnts[10], ncons, maxcon, maxconcnt;
	WORD remMask, OKmask;
	for(row = 0, baseConsRow = 0; row < 9 ; row++) {
		for(col = 0, baseConsCol = 0; col < 9 ; col++) {
			for(i = 0; i < 10 ; i++) con_cnts[i] = 0;
			ncons = maxcon = maxconcnt = 0;
			if((row == 0) && (col == 2)) {
				dbcnt++;
			}
			remMask = 0;
			if((col %3) != 0) {	// if not first col in 3x3 block, check constraint with box to left
				...
				if((con > 0) && (con_cnts[con] > maxconcnt)) maxconcnt = con_cnts[con];
			}
			...
			if(maxcon != 0) { // constraints
				if((con_cnts[1] > 0) || (maxconcnt > 1)) {	// 1 cannot have quotiwent of 1 or 2 of any
					remMask |= valid_masks[1];
					if((con_cnts[1] > 1) || (maxconcnt > 3)) { // 2 can only have quot of 1 with 3 and 2: 1 4 5
						remMask |= valid_masks[2];
						if(maxconcnt > 3) { // 3 can have quot of 1 wiht 2,4,5, 2 -> 6,7,8 no otherwith more than 2
							remMask |= valid_masks[3];
						}
					}
				}
				if(maxcon > 4) {	// one must be 1 and the other con
					for(con = 5; con <= maxcon  ; con++) {
						if(con_cnts[con] > 0) {
							OKmask = valid_masks[1] | valid_masks[con];
							remMask |= ~OKmask;
						}
					}
				} else if(maxcon == 4) { // either 1 & 4 or 2 & 8 or 9
					remMask |= valid_masks[3] | valid_masks[5] | valid_masks[6] | valid_masks[7];
				}
...
WORD checkConstraint(int constraint, WORD baseMask, WORD chkMask)
{
	...
	int i, j;
	OKmask = 0;
	if(constraint == 0) { // no restricition
		return 0;
	}
	...
	for(i = 1; i <= 9 ; i++) {
		if(chkMask & valid_masks[i]) {
			for(j = i/(constraint+1) + 1; j <= i/constraint; j++) {
				if((j > 0) && (j != i)) OKmask |= valid_masks[j];
			}
		}
	}
	// if value in chkMask is the smaller, values in base can be in range
	// chkval*constraint. to (chkval+1)*constraint-1 
	for(i = 1; i <= 9 ; i++) {
		if(chkMask & valid_masks[i]) {
			for(j = i*constraint; j <= ((i)*(constraint+1) - 1); j++) {
				if((j <= 9) && (j != i)) OKmask |= valid_masks[j];
			}
		}
	}
			...
			for(col = 0, baseConsCol = 0; col < 9 ; col++) {
				...
				if(pss->val_set[row][col] == 0) {	// if we have not already set this box in search see if it can change
					...
					if(baseMask == 0) {	// not already set and no values available for box
						fprintf(stderr, "cc row %d col %d ccol %d crow %d\n",
							row, col, baseConsCol, baseConsRow);
						return -1;
					}
...
/*
 * chopose how to branch for the next sudoku depth first search step
 ...
 * then chack quotient constraints
